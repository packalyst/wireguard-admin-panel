    ...
    async def handle_stream(self, stream, address):
        ...
        if ":" in auth and EnumSocks5AuthMethod.PASSWORD in auth_request.auth_methods:
            ...
            if (
                ...
                # Panel marker: feeds the firewall jail (brute-force bans).
                utils.logger.info(
                    "TURBOTUNNELS_AUTH_FAIL SRC=%s USER=%s" % (address[0], username)
                )
        ...
        # Panel marker: records the authenticated connection for the logs page.
        # Internal health probes are dropped by the api-side source filter.
        utils.logger.info(
            "TURBOTUNNELS_CONN SRC=%s USER=%s DST=%s DPORT=%d"
            % (
                address[0],
                auth.split(":", 1)[0] if auth else "-",
                target_address[0],
                target_address[1],
            )
        )
