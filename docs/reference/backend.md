...
- **Migration status:** interval jobs — `cloudflare-ips`, `session-cleanup`, `ratelimit-cleanup`, `firewall-cleanup`, `firewall-l3-sampler`, `logs-country`, `logs-cleanup`, `vpn-traffic-sync`, `turbotunnels-guard-cleanup`, `turbotunnels-log-streamer`, `sudo-watcher`; daily `geo-update`; read-only daemons `ws-hub`, `server-stats-collector`, `firewall-jails`, and one `log-watcher:<source>` per enabled log source. The only remaining private goroutines are the ones without a single clean schedule (the per-jail monitors inside the jail engine, the per-source tail loops themselves, and request-scoped fan-out); they're surfaced via the daemon rows above.
- **Lifecycle:** `main` runs the supervisor under a cancellable context and cancels it on shutdown *before* `database.Close()`, so no tick fires an Exec against a closed pool. `Init` is idempotent; `RunNow` is throttled per routine (`manualRunCooldown`); the broadcaster is stored atomically.
